<?php
add_image_size( 'News', 350, 150, true ); 
add_image_size( 'News-small', 100, 100, true );
...
class rpwe_widget extends WP_Widget
{
				...
				<?php foreach ($rpwewidget as $post) : setup_postdata($post); ?>
					...
					<li class="rpwe-clearfix">
						...
						<?php if (has_post_thumbnail() && $thumb == true) { ?>
							...
							<a class="post-image" href="<?php the_permalink(); ?>" title="<?php printf(esc_attr__('Permalink to %s', 'rpwe'), the_title_attribute('echo=0')); ?>" rel="bookmark">
								<?php
								if (current_theme_supports('get-the-image'))
									get_the_image(array('meta_key' => 'Blog', 'height' => $thumb_height, 'width' => $thumb_width, 'image_class' => 'rpwe-alignleft', 'link_to_post' => false));
								else
						...
						<?php if ($excerpt == true) { ?>
                            ...
                            <div class="rpwe-readmore"><a href="<?php the_permalink(); ?>" title="Read more about <?php the_title(); ?>">Read more</a></div>
						<?php } ?>
	...
	function form($instance)
	{
		...
		<p>
			<label for="<?php echo $this->get_field_id('offset'); ?>"><?php _e('Offset:', 'rpwe'); ?></label>
			<input id="<?php echo $this->get_field_id('offset'); ?>" name="<?php echo $this->get_field_name('offset'); ?>" value="<?php echo $offset; ?>" />
		</p>
